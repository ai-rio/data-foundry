#!/usr/bin/env python3
"""
Pilot Test Harness - Run Kaggle datasets through EXISTING ingestion pipeline.

NO MODIFICATIONS to the pipeline. Just measure how it performs on real data.

Purpose:
- Test pipeline works end-to-end with real datasets
- Measure processing time per vertical
- Identify any errors/issues before frontend is built
- Validate system assumptions from Phase 0 research

Output:
- pilot_test_results/ directory with results per vertical
...
async def run_pilot_test(config: PilotTestConfig) -> PilotTestResult:
    ...
    try:
        ...
        if config.enable_validation:
            ...
            try:
                valid_data, invalid_data = validate_schema(extracted)
...
async def main():
    ...
    tests = [
        PilotTestConfig(
            vertical="fintech",
            dataset_path=str(PROJECT_ROOT / "data/pilot_test_datasets/fintech/Banking_Transactions_USA_2023_2024.csv"),
            sample_size=1000,  # Start with 1000 to test quickly
        ),
        PilotTestConfig(
            vertical="healthcare",
            dataset_path=str(PROJECT_ROOT / "data/pilot_test_datasets/healthcare/healthcare_dataset.csv"),
            sample_size=1000,
        ),
        PilotTestConfig(
            vertical="ecommerce",
            dataset_path=str(PROJECT_ROOT / "data/pilot_test_datasets/ecommerce/Phones.csv"),
